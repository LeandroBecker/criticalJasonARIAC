...
public class RosEnv extends Environment {
    ...
    public void init(String[] args) {
		...
		/* Subscriber for getting the PERCEPTION message */
		bridge.subscribe(SubscriptionRequestMsg.generate("/ariac_human/critical_percept") 
				.setType("std_msgs/Bool")
				.setThrottleRate(1)
				.setQueueLength(1),
			new RosListenDelegate() {
				public void receive(JsonNode data, String stringRep) {
					MessageUnpacker<PrimitiveMsg<Boolean>> unpacker = new MessageUnpacker<PrimitiveMsg<Boolean>>(PrimitiveMsg.class);
					PrimitiveMsg<Boolean> msg = unpacker.unpackRosMessage(data);
					logger.info("Crical Perception arrived!");
					if (msg.data){
						//clearPercepts("human");
		...
		); // END bridge.subscribe(..."/ariac_human/critical_percept")
	} // END init()
    ...
    public boolean executeAction(String agName, Structure act) {
		...
		else if (act.getFunctor().equals("trigger_c_reaction")) { 
			critical_reaction(); 
		}
		else {
	...
	// Published topic is read by human_control.py
	public void critical_reaction() { 
		Publisher cReaction_h = new Publisher("/ariac_human/critical_react", "std_msgs/Bool", bridge);	
		cReaction_h.publish(new Bool(true));
		logger.info("ENV published Critical Reaction");	
	}
